...
const HELP_CONTENT: Record<string, { title: string; desc: string; steps: string[] }> = {
  ...
  "/admin/analytics": {
    ...
    desc: "مراقبة أداء المنتجات وأكثرها جلباً للاهتمام في المتجر.",
    steps: [
      "متابعة إجمالي زيارات كل منتج على حدة.",
      "معرفة أكثر المنتجات التي تلفت الانتباه وتجذب المشاهدات.",
      "استخدام هذه البيانات لإطلاق حملات تسويقية وتخفيضات.",
      "يتم احتساب المشاهدات الفريدة للحفاظ على دقة التحليل."
    ]
  },
  "/admin/coupons": {
    title: "أكواد الخصم",
    desc: "إدارة وتتبع أكواد الخصم (الكوبونات) المتاحة للزبائن.",
    steps: [
      "إنشاء كود خصم جديد مع تحديد نسبة الخصم.",
      "تحديد الحد الأقصى لقيمة الخصم لحماية الأرباح.",
      "مراقبة عدد مرات الاستخدام لكل كود.",
      "إيقاف تفعيل الكوبونات أو مسحها."
    ]
  },
  "/admin/team": {
    title: "إدارة الفريق",
    desc: "منح وإدارة الصلاحيات للمدراء والمشرفين على المتجر.",
    steps: [
      "دعوة مدير جديد وتحديد صلاحياته (مدير رئيسي أو عادي).",
      "إرسال دعوات التسجيل عبر البريد الإلكتروني.",
      "تتبع حالة المدراء (نشط / قيد الانتظار).",
      "إزالة صلاحيات المشرفين السابقين بسهولة."
    ]
  },
  "/admin/logs": {
    title: "سجل العمليات",
    desc: "تتبع كل الحركات والتعديلات التي تتم في لوحة التحكم.",
    steps: [
      "معرفة من قام بتعديل منتج أو تغيير حالة طلب.",
      "تتبع أوقات الأنشطة وتعديلات المشرفين.",
      "مراقبة الأنشطة الحساسة لضمان أمان المتجر.",
      "الاحتفاظ بسجل شفاف وموثوق لإدارة المتجر."
    ]
  }
};
...
export default function AdminHelp() {
  ...
  const baseKey = Object.keys(HELP_CONTENT).reduce((longest, current) => {
    if (pathname.startsWith(current) && current.length > longest.length) {
      return current;
    }
    return longest;
  }, "");

  const content = HELP_CONTENT[baseKey] || {
